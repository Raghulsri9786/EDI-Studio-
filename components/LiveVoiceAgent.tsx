...
import { Mic, X, Cpu, AlertTriangle, Files } from 'lucide-react';
...
const LiveVoiceAgent: React.FC<LiveVoiceAgentProps> = ({ isOpen, onClose, files }) => {
      ...
      <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-sm w-full relative flex flex-col items-center gap-6 border-t-4 border-indigo-600">
        ...
        <div className="text-center">
          <h3 className="text-xl font-bold text-slate-800">
            {error ? "Voice Error" : (isConnected ? (isSpeaking ? "Speaking..." : "Listening...") : "Connecting...")}
          </h3>
          ...
          <div className="mt-4 flex items-center justify-center gap-2 bg-slate-100 px-3 py-2 rounded-lg border border-slate-200">
             ...
             <span className="text-xs font-semibold text-slate-700">
               {files.length > 0 ? `${files.length} Files in Context` : "No context files"}
             </span>
          </div>
          {error && <p className="text-xs text-red-600 mt-4 font-bold bg-red-50 px-3 py-2 rounded border border-red-100">{error}</p>}
        </div>
        <div className="flex gap-4 w-full">
          <button onClick={onClose} className="w-full py-2.5 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 font-bold text-sm transition-colors">
            Close
          </button>
        </div>
      </div>
